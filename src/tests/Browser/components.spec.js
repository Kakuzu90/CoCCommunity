...
test('form feedback, searchable select, fallback, and dismiss controls work', async ({ page }) => {
    ...
    await page.getByLabel('Filter Searchable category options').fill('trophy');
    await expect(page.locator('#search-category option[value=war]')).toHaveAttribute('hidden');
    await page.getByLabel('Searchable category', { exact: true }).selectOption('trophy');
